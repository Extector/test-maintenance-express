"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.VehicleModelController = void 0;
const common_1 = require("@nestjs/common");
const create_vehicle_model_usecase_1 = require("./usecases/create-vehicle-model.usecase");
const list_vehicle_models_usecase_1 = require("./usecases/list-vehicle-models.usecase");
let VehicleModelController = class VehicleModelController {
    listVehicleModelsUseCase;
    createVehicleModelUseCase;
    constructor(listVehicleModelsUseCase, createVehicleModelUseCase) {
        this.listVehicleModelsUseCase = listVehicleModelsUseCase;
        this.createVehicleModelUseCase = createVehicleModelUseCase;
    }
    async list() {
        return this.listVehicleModelsUseCase.execute();
    }
    async create(body) {
        return this.createVehicleModelUseCase.execute({ name: body.name });
    }
};
exports.VehicleModelController = VehicleModelController;
__decorate([
    (0, common_1.Get)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], VehicleModelController.prototype, "list", null);
__decorate([
    (0, common_1.Post)(),
    (0, common_1.HttpCode)(201),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], VehicleModelController.prototype, "create", null);
exports.VehicleModelController = VehicleModelController = __decorate([
    (0, common_1.Controller)("vehicle-models"),
    __metadata("design:paramtypes", [list_vehicle_models_usecase_1.ListVehicleModelsUseCase,
        create_vehicle_model_usecase_1.CreateVehicleModelUseCase])
], VehicleModelController);
//# sourceMappingURL=vehicle-model.controller.js.map